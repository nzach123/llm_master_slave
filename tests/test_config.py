...
def test_load_config(tmp_path):
    ...
    with patch("core.config.load_dotenv"): # Don't load real .env
...
def test_load_config_defaults(tmp_path):
    ...
    with patch("core.config.load_dotenv"): # Don't load real .env
        ...
        # Ensure we don't accidentally get prompted for API key
        with patch("getpass.getpass", return_value="dummy"):
            config = load_config()
