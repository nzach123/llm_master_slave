...
def test_spine_retry_logic(mock_client_class, mock_check):
    ...
    step = DispatchStep(agent_name="coder", task_description="task", context_files=[])
    
    with patch.object(spine.coder, "handle_task") as mock_handle:
        # Fail once, then succeed
        mock_handle.side_effect = [Exception("Fail"), SpokeResponse(thoughts="Success", tool_calls=[])]
